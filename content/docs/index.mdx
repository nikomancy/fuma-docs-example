...
1. Have `employee__c` and `address__c` objects defined in your Vault instance.
1. Have a basic understanding of [Vault Objects](https://platform.veevavault.help/en/lr/15298/).
1. Have a username, password, and Vault DNS.
...
// Store your username and password in a .env file and import it into your 
// application using the process.env value.
...
> For more information on authentication, see the [Authentication docs](https://developer.veevavault.com/docs/) and [Authentication reference](https://developer.veevavault.com/api/24.2/#authentication).
...
The following API call to the `/vobjects/employee__c` endpoint will create vault objects corresponding to each of the entries in the employees.csv file.
...
## Create addresses
Vault Objects are flexible and track associations between objects defined in their schema automatically. For example, when we import the address objects contained in the addresses.csv file, Vault will automatically associate them with their related users in the system by way of the `employee__c.name__v` field.

The following call to the `/vobjects/address__c` endpoint will create vault objects for each of the addresses in the addresses.csv file.
...
```ts
const { API_VERSION, DNS_DOMAIN } = process.env;
const sessionId = 'A2A45E3BA1B53F2536A235FEA904D7615EDE3A6206C31D65AB10B7615D684F87D2B4B51899757044D35365F730E60E070D82E15F874867478223961A9050881F';
...
```ts
// Import the environment variables USER_NAME, PASSWORD, API_VERSION, DNS_DOMAIN
const { API_VERSION, DNS_DOMAIN } = process.env;
const sessionId = ''; // Add your sessionId here
